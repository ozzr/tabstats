...
header labels like "High Risk  (n=30)" can never be truncated by a narrow
data column.
...
def render_text_table(
    ...
    for k, row_vals in enumerate(flat_df.values):
        ...
        # ── Section header row (full-width separator with label) ──────────
        if first_val.startswith("\u2500\u2500\u2500"):
            label = first_val.lstrip("\u2500 ").rstrip("\u2500 ")
            sec_cells: List[SCell] = [SCell("\u2500", label, n_cols)]
            lines.append(_render_sep_row(sec_cells, widths))
            continue
        ...
        sep_cells_d: List[SCell] = [SCell("-") for _ in range(n_cols)]
        if k in pval_map:
            p_str, test_str = pval_map[k]
            if lay.p_idx    is not None: sep_cells_d[lay.p_idx]    = SCell("-", p_str)
            if lay.test_idx is not None: sep_cells_d[lay.test_idx] = SCell("-", test_str)
        elif k in blank_set:
            if lay.p_idx    is not None: sep_cells_d[lay.p_idx]    = SCell(" ")
            if lay.test_idx is not None: sep_cells_d[lay.test_idx] = SCell(" ")
        lines.append(_render_sep_row(sep_cells_d, widths))
